...
#define UVC_LCD_PREVIEW_ENABLE 1U
#define UVC_LCD_RENDER_INTERVAL 3U
...
static void uvc_frame_thread_entry(void *arg)
{
    ...
    uint16_t stream_w;
    uint16_t stream_h;
    uint8_t ai_started = 0U;

#if UVC_LCD_PREVIEW_ENABLE
    ...
    while (uvc_app_running) {
        ...
        if (!ai_started) {
            uint16_t ai_w = stream_w ? stream_w : uvc_cam_w;
            uint16_t ai_h = stream_h ? stream_h : uvc_cam_h;
            USB_LOG_INFO("UVC AI start after first frame: %ux%u req=%ux%u size=%u\r\n",
                         (unsigned)ai_w, (unsigned)ai_h,
                         (unsigned)uvc_cam_w, (unsigned)uvc_cam_h,
                         (unsigned)frame->frame_size);
            (void)uvc_ai_app_start(ai_w, ai_h);
            ai_started = 1U;
        }
...
#if UVC_LCD_PREVIEW_ENABLE
        if ((frame_count % UVC_LCD_RENDER_INTERVAL) == 0U) {
            /* render frame to LCD at a reduced rate to keep HTTP responsive */
